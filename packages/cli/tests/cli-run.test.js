...
test('kobean help returns 0', async () => {
  const code = await runCli([runtime, 'kobean', 'help']);
  assert.equal(code, 0);
});

test('kobean status checks daemon connectivity', async () => {
  const code = await runCli([runtime, 'kobean', 'status']);
...
test('kobean report without file returns error code 1', async () => {
  const code = await runCli([runtime, 'kobean', 'report']);
  assert.equal(code, 1);
});

test('kobean run forwards child process exit code', async () => {
  // Exits with 42
  const code42 = await runCli([runtime, 'kobean', 'run', '--cmd', `"${runtime}" -e "process.exit(42)"`]);
  assert.equal(code42, 42);

  // Exits with 0
  const code0 = await runCli([runtime, 'kobean', 'run', '--cmd', `"${runtime}" -e "process.exit(0)"`]);
